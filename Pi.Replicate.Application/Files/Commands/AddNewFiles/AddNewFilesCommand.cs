...
namespace Pi.Replicate.Application.Files.Commands.AddNewFiles
{
	...
	public class AddNewFilesCommandHandler : IRequestHandler<AddNewFilesCommand, List<File>>
	{
		...
		private const string _insertStatement= "INSERT INTO dbo.[File](Id,FolderId, Name, Size,AmountOfChunks,Hash,Status,LastModifiedDate,Path,Signature, Source) VALUES(@Id,@FolderId,@Name,@Size, @AmountOfChunks, @Hash, @Status, @LastModifiedDate,@Path, @Signature, @Source)";
		...
		public async Task<List<File>> Handle(AddNewFilesCommand request, CancellationToken cancellationToken)
		{
			using (_database)
			{
				...
				foreach (var newFile in request.NewFiles)
				{
					...
					await _database.Execute(_insertStatement, new { file.Id, file.FolderId, file.Name, file.Size, file.AmountOfChunks, file.Hash, file.Status, file.LastModifiedDate, file.Path, file.Signature, file.Source });
