...
namespace Pi.Replicate.Application.Files.Queries.GetLatestVersionOfFile
{
    ...
    public class GetLatestVersionOfFileQueryHandler : IRequestHandler<GetLatestVersionOfFileQuery, File>
    {
        ...
        private const string _selectStatement = "SELECT TOP 1 Id, FolderId,Version, LastModifiedDate, Name,Path, Signature,Size,Source FROM dbo.[File] WHERE FolderId = @FolderId Path = @Path order by Version desc";
